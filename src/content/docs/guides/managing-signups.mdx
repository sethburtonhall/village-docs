...
- **Event Analytics** — the Recent sign-ups feed and Top events list on the dashboard also update automatically <Badge text="Individual" variant="note" />
...
4. A removal dialog appears

![Remove signup icon](../../../assets/screenshots/managing-signups/admin-remove.png)

In the removal dialog, you must:

- **Enter a reason or note** — explain why you're removing this attendee (e.g. "Attendee requested cancellation", "Reached capacity — removing waitlisted signup", or "Duplicate registration")

You can also:

- Choose whether to **notify the attendee by email** (checked by default)

Click **Remove** to confirm. The attendee's slot is immediately returned to the event's available count. If you chose to notify them, they receive an email with your note included.

<Aside>
  The removal note is stored in your admin records for your reference. The
  attendee sees a brief version of it in their notification email if you choose
  to notify them.
</Aside>
...
- Add the event to **Google Calendar**, **Outlook**, or **Apple Calendar / iCal** directly from the page — no need to find their original confirmation email
...
## Checking in attendees :badge[Organization]{variant="tip"}
...
Individual plan users and above also see sign-up activity on their dashboard:
